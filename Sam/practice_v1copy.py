...
svs_src = r'\\motherserverdw\Kyu_Sync\Aging\data\fb_detection_practice'
...
for fn in fns:
    ...
    for regionidx,region in enumerate(rois):
        ...
        bw_img.save(os.path.join(dst, fn + '_SZ_filtered_practice_{:d}.tif'.format(regionidx)))
        #3 AR filter
        minAR = 2.5
        maxAR = 7.5
        start=time()
        ...
        bw = (labeled_bw > 0) * 255
        bw_img = Image.fromarray(bw).convert('1')
        bw_img.save(os.path.join(dst, fn + '_AR_filtered_practice_{:d}.tif'.format(regionidx)))
        ...
        bw = (labeled_bw > 0) * 255
        bw_img = Image.fromarray(bw).convert('1')
        bw_img.save(os.path.join(dst,fn+'_Dist_filtered_practice_{:d}.tif'.format(regionidx)))
        ...
        bw = (labeled_bw>0)*255
        bw_img = Image.fromarray(bw).convert('1')
        bw_img.save(os.path.join(dst,fn+'_SF_filtered_practice_{:d}.tif'.format(regionidx)))
        ...
        df.to_csv(os.path.join(dst,fn+'_parameters_practice_{:d}.csv'.format(regionidx)))